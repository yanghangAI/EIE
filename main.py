...
def model_init(stage, load_path='', load_opt=True):
    net = EIE(in_1=3, in_2=3)
    if stage == 0:
        lr = c.lr
...
def main(attack_method, step, load_path='', start_epoch=0, end_epoch = 1600, lam=(1.0, 1.0)):
    ...
    net, optim, weight_scheduler = model_init(stage=step, load_path=load_path, load_opt=load_opt)
